        ...
        if(data)
        {
            ...
            if(f)
            {
                let x=await cm.find({uid:data._id})
                 let count=x.length
                res.json({"token":jwt.sign({"_id":data._id},"12345"),"uid":data._id,"name":data.name,"role":data.role,"count":count})
            }
            else{
...
const transporter = nodemailer.createTransport({
  ...
  secure: true, 
  auth: {
    user: "pappurujayalakshmi4@gmail.com",
    pass: "rlhyrovufpweafom",
  },
});
...
let sendotp=async(req,res)=>{
    ...
    let info = await transporter.sendMail({
        from: '"Maddison Foo Koch" <pappurujayalakshmi4@gmail.com>',
        to: req.params.eid,
...
module.exports={login,adduser,sendotp,resetpwd}